...
public struct AgentSessionDurableState: Codable {
    ...
    public let archiveState: AgentArchiveState?
...
extension AgentSimulationSession {
    public func checkpointReadiness(
        ...
        // Promise and obligation state is already durable social state. Open
        // proposals, accepted contracts awaiting consideration, and open debt
        // are checkpoint-safe because no adapter mutation is in flight.
    ...
    static func validateDurableState(_ state: AgentSessionDurableState) throws {
        ...
        let latestSchema = archiveSchema || writingSchema || renewableSchema || independentReceiptSchema
        ...
        guard (state.schemaVersion == AgentCheckpointSchema.currentVersion
                ...
                || (knowledgeSchema && state.knowledgeGraphState != nil)
                || (languageSchema
                ...
                || (archiveSchema && state.knowledgeGraphState != nil
                    && state.languageState != nil && state.writingState != nil
        ...
        guard (archiveSchema || writingSchema || knowledgeSchema || languageSchema || oralSchema
              ...
              (longDistanceCommunicationSchema
                ...
                == (state.longDistanceCommunicationState != nil),
              (writingSchema || archiveSchema) == (state.writingState != nil),
              archiveSchema == (state.archiveState != nil) else {
            throw AgentCheckpointError.unsupportedSchema(state.schemaVersion)
        }
        ...
        if let agriculture = state.agricultureState {
            do {
                ...
                try validateAgricultureState(
                    ...
                    population: state.populationRegistry,
                    mortality: state.mortalityState,
                    ecologicalObservations: state.ecologicalObservationState,
                    clock: state.clock,
                    causalLatestSequence: state.causalLedger.latestSequence,
                    causalDroppedEventCount: state.causalLedger.droppedEventCount,
                    causalEvents: state.causalLedger.events
