    ...
    func handleBuild(
        ...
        if subcommand == "status" {
            ...
            let placed = project.map {
                "wood:\($0.installedMaterialTotals.count(of: .wood)),stone:\($0.installedMaterialTotals.count(of: .stone))"
            } ?? "wood:0,stone:0"
        ...
        if subcommand == "clear" {
            ...
            do {
                ...
                let materialResult = project.materialAuthority == .physicalCustody
                    ? "real placed materials were not converted into abstract stock"
                    : "coarse escrow materials refunded"
                return success("Construction cleared; restored \(restored) owned project blocks; \(materialResult).")
            } catch {
        ...
        do {
            ...
            lastConstructionReason = "safe site selected read-only; real custody authoritative"
