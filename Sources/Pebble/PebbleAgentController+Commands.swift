...
extension PebbleAgentController {
    func handleCommand(_ arguments: [String], world: World, player: Player) -> PebbleAgentCommandResult {
        ...
        case "help":
            ...
            return success("/lab session: start stop clear | lifecycle <on|status|clear> reproduction <on|off|status> births status kinship <on|status> household <on|status> care <on|status|proof proximity-setup|physical-food-setup> childhood <on|status|delegate|reassign|proof guardian-separation> family <on|status|propose|accept|separate|lineage|house-found|house-cofound|house-leave> estates <on|status|accept|settle|proof rollback> | time control: pause resume step speed <1|2|4|8> reset | inspection: status focus <agentId|next> next follow <agentId|focus|next|off> overlay <off|compact|full> observer <open|close|status|next|previous|select|individual|global|reason|event> causality <status|tail <1...20>> scale status | persistence: checkpoint <status|list|save|load|delete> replay <status|start|stop|verify> persistence-reconciliation <setup|status|cleanup> | movement: movement <on|off> embodiment proof | interaction: interaction <setup|setup distant <2...8>|harvest|status|auto on|auto off> gateway proof material proof rights <proof|status|clear> harvest proof construction proof | economy: economy <setup|auto on|auto off|status|clear> | survival: survival <on|off|status> physical-food-survival <on|off|status|proof> | natural: natural <on|off|status|scan> | ecology: ecology <on|off|status|scan|clear> forage status | ecological-observation <on|status|scan|proof> agriculture <on|status|proof> wild-subsistence <on|status|proof [setup|fish|hunt|gather|final]> work-professions <on|refresh|match|record|crisis|resume|status|final> | mortality: mortality <on|off|status|clear> homeostasis <on|status|proof> exits status | build: build <setup|auto on|auto off|status|clear> | social: social <on|off|status|clear> | physical: physical <on|off|status|clear> | cooperation: cooperation <on|off|status|clear> | population: population <on|off|status|clear> migration <admit|status> civ39 <setup|status|migrate|proof> | settlement: settlement <on|off|status|clear> | demo: demo [start|stop|status]")
        case "demo":
        ...
        case "observer":
            return handleObserver(Array(arguments.dropFirst()), world: world)
        case "observer-proof":
            return handleObserverProof(Array(arguments.dropFirst()), world: world)
        case "harvest":
        ...
        case "renewable-subsistence":
            return handleRenewableSubsistence(
                Array(arguments.dropFirst()), world: world
            )
        ...
        case "childhood":
            return handleChildhood(Array(arguments.dropFirst()))
        case "family":
            return handleFamily(Array(arguments.dropFirst()), world: world)
        case "estates":
            return handleEstates(Array(arguments.dropFirst()), world: world)
        case "skills":
        ...
        case "persistence-reconciliation":
            return handlePersistenceReconciliation(
                Array(arguments.dropFirst()), world: world
            )
        case "replay":
        ...
        case "status":
            ...
            let homeostasisStatus = session.homeostasisEnabled
                ? " homeostasis=on" : ""
            ...
            let childhoodStatus = session.childhoodV2Enabled
                ? " childhood=on" : ""
