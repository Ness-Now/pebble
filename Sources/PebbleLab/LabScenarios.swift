...
let supportedScenarios = ["empty", "chunk_smoke", "agent_smoke", "agents_basic", "seek_safety_smoke", "long_run_smoke", "regression_smoke", "physical_placeholder_smoke", "physical_sync_smoke", "core_entity_smoke"]

func validateScenario(_ scenario: String) {
    guard supportedScenarios.contains(scenario) else {
        fail("unsupported scenario: \(scenario). Currently supported: empty, chunk_smoke, agent_smoke, agents_basic, seek_safety_smoke, long_run_smoke, regression_smoke, physical_placeholder_smoke, physical_sync_smoke, core_entity_smoke")
    }
}
...
func prepareScenario(_ options: Options, world: World) -> ScenarioResult {
    ...
    switch scenario {
    case "chunk_smoke", "agent_smoke", "agents_basic", "seek_safety_smoke", "long_run_smoke", "regression_smoke", "physical_placeholder_smoke", "physical_sync_smoke", "core_entity_smoke":
        registerAllBlocks()
        registerAllBiomes()

        let chunkRadius = (scenario == "agent_smoke" || scenario == "agents_basic" || scenario == "seek_safety_smoke" || scenario == "long_run_smoke" || scenario == "regression_smoke" || scenario == "physical_placeholder_smoke" || scenario == "physical_sync_smoke" || scenario == "core_entity_smoke") && !options.chunkRadiusProvided ? 1 : options.chunkRadius
        var adoptedChunks: [AdoptedChunk] = []
        ...
        if scenario == "agent_smoke" {
            ...
            result.agents = [agent]
            _ = result.physicalBridge.spawnPlaceholder(for: agent, tick: 0)
        } else if scenario == "core_entity_smoke" {
            var agent = makeLabAgent(id: "agent_0", x: 8, z: 8, world: world)
            agent.needs.curiosity = 0.9
            result.agents = [agent]
...
func makeWorldSnapshot(
    ...
    guard (options.scenario == "chunk_smoke" || options.scenario == "agent_smoke" || options.scenario == "agents_basic" || options.scenario == "seek_safety_smoke" || options.scenario == "long_run_smoke" || options.scenario == "regression_smoke" || options.scenario == "physical_placeholder_smoke" || options.scenario == "physical_sync_smoke" || options.scenario == "core_entity_smoke"),
