...
func prepareScenario(_ options: Options, world: World) -> ScenarioResult {
    ...
    switch scenario {
    case "terrain_semantics_fixture_smoke", "terrain_traversability_fixture_smoke", "terrain_pathfinding_fixture_smoke", "terrain_pathfinding_column_positive_smoke", "terrain_path_live_movement_smoke", "terrain_path_movement_fixture_smoke", "terrain_collision_fixture_smoke":
        return ScenarioResult()
    case "chunk_smoke", "agent_smoke", "agents_basic", "seek_safety_smoke", "long_run_smoke", "regression_smoke", "physical_placeholder_smoke", "physical_sync_smoke", "core_entity_smoke", "physical_behavior_smoke", "physical_behavior_multi_smoke", "physical_movement_denied_smoke", "physical_movement_find_occupable_smoke", "world_observation_smoke", "world_observation_multi_smoke", "terrain_scan_smoke", "terrain_scan_edge_smoke", "terrain_column_scan_smoke", "terrain_column_scan_edge_smoke", "terrain_pathfinding_column_smoke", "terrain_pathfinding_column_edge_smoke", "terrain_collision_live_readonly_smoke":
        registerAllBlocks()
        registerAllBiomes()

        let chunkRadius = (scenario == "agent_smoke" || scenario == "agents_basic" || scenario == "seek_safety_smoke" || scenario == "long_run_smoke" || scenario == "regression_smoke" || scenario == "physical_placeholder_smoke" || scenario == "physical_sync_smoke" || scenario == "core_entity_smoke" || scenario == "physical_behavior_smoke" || scenario == "physical_behavior_multi_smoke" || scenario == "physical_movement_denied_smoke" || scenario == "physical_movement_find_occupable_smoke" || scenario == "world_observation_smoke" || scenario == "world_observation_multi_smoke" || scenario == "terrain_collision_live_readonly_smoke" || isTerrainColumnScanScenario(scenario) || isTerrainScanScenario(scenario)) && !options.chunkRadiusProvided ? 1 : options.chunkRadius
        var adoptedChunks: [AdoptedChunk] = []
